function [T,Tempo,flag]=transient3_pruning(sat,T_0,dt,r_dc,sim_data,Tempo,G0_Irr,G_c,G_hc,...
    Q_0,Qdiss_dyn,casevalue,T_op,opt_st)

flag = 0; % solution validity: 0 = valid, 1 = invalid
N_instants = ((length(r_dc)-1)*sim_data.n_orbit)-1;
sigma=5.67e-8;
T=zeros(sat.node.total_node,length(r_dc)*sim_data.n_orbit);
T(:,1)=T_0;
switch casevalue
    case 1
...
for k=1:N_instants
    ...
    % check solution validity every step, then every 10 steps, then every
    % 100 steps (the last step is not checked)
    if ((rem(k,100)==0 || (k<100 && rem(k,10)==0) || k<10) && k~=N_instants) && opt_st.transient_pruning == 1
        T_Celsius=T-273.15;
        Tmin_max=[min(T_Celsius(:,1:k+1),[],2), max(T_Celsius(:,1:k+1),[],2)];
        % 0 = valid, 1 = invalid
        [flag,~] = validate_solution(sat,Tmin_max,[],1,T_op,[],[],0);
        if flag == 1
            break;
        end
    end
