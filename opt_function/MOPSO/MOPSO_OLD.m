...
% ----------------------------------------------------------------------- %
function REP = MOPSO_OLD(params,MultiObj)

    writerObj = VideoWriter('MOPSO_PF.avi');
    ...
    while ~stopCondition
        ...
        if(size(POS_fit,2)==2)
            ...
            fontsize(h_fig, 36, "points") % 24
            mrk_size = 18; % 12
            % h_par = plot(POS_fit(:,1),POS_fit(:,2),'o','MarkerEdgeColor',[1, 0, 0],'MarkerFaceColor', [229, 240, 255]./256, 'MarkerSize', mrk_size, 'LineWidth', mrk_size); hold on;
            h_rep = plot(REP.pos_fit(:,1),REP.pos_fit(:,2),'o','MarkerEdgeColor',[0, 38, 79]./256,'MarkerFaceColor', [229, 240, 255]./256, 'MarkerSize', mrk_size, 'LineWidth', 2); hold on;
            try
                % set(gca,'xtick',REP.hypercube_limits(:,1)','ytick',REP.hypercube_limits(:,2)');
                % axis([min(REP.hypercube_limits(:,1)) max(REP.hypercube_limits(:,1)) ...
                %       min(REP.hypercube_limits(:,2)) max(REP.hypercube_limits(:,2))]);
            ...
            if(isfield(MultiObj,'truePF'))
                try delete(h_pf); end
                h_pf = plot(MultiObj.truePF(:,1),MultiObj.truePF(:,2),'.','MarkerFaceColor',[146,208,80]./256, 'MarkerEdgeColor',[146,208,80]./256,'MarkerSize',mrk_size); hold on;
            end
            set(gca, 'Color','w', 'XColor',[0, 38, 79]./256, 'YColor',[0, 38, 79]./256)
            grid on; xlabel('Objective 1'); ylabel('Objective 2');            
            axis square;
