% ----------------------------------------------------------------------- %
% Example of use of the funcion MOPSO_OLD.m, which performs a Multi-Objective %
% Particle Swarm Optimization (MOPSO_OLD), based on Coello2004.               %
% ----------------------------------------------------------------------- %
...
switch MultiObjFnc
    ...
    case 'Kursawe'          % Kursawe 
        MultiObj.fun = @(x) [-10.*(exp(-0.2.*sqrt(x(:,1).^2+x(:,2).^2)) + exp(-0.2.*sqrt(x(:,2).^2+x(:,3).^2))) + 20, ...
                             sum(abs(x).^0.8 + 5.*sin(x.^3),2) + 11.6265];
        MultiObj.nVar = 3;
...
params.u_mut = 0.2;     % Uniform mutation percentage 0.5

% MOPSO_OLD
REP = MOPSO_OLD(params,MultiObj);

% MOPSO_OLDER
% REP = MOPSO_OLDER(params,MultiObj);
