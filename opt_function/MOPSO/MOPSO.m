...
% ----------------------------------------------------------------------- %
function [x_pareto, fval_pareto, x_all, fval_all] = MOPSO(params,MultiObj)
    ...
    if(size(POS_fit,2)==2) && params.plot_results
%             figure(h_fig); delete(h_par); delete(h_rep);
        plot(params.ax,POS_fit(:,1),POS_fit(:,2),'or');
        hold(params.ax,"on");
        plot(params.ax,REP.pos_fit(:,1),REP.pos_fit(:,2),'ok');
        hold(params.ax,"on");
%             try
%                 set(params.ax,'xtick',REP.hypercube_limits(:,1)','ytick',REP.hypercube_limits(:,2)');
%                 axis(params.ax,[min(REP.hypercube_limits(:,1)) max(REP.hypercube_limits(:,1)) ...
%                       min(REP.hypercube_limits(:,2)) max(REP.hypercube_limits(:,2))]);
%             end
        if(isfield(MultiObj,'truePF'))
%                 try delete(h_pf); end
            plot(params.ax,MultiObj.truePF(:,1),MultiObj.truePF(:,2),'.','color',0.8.*ones(1,3));
            hold(params.ax,"on");
        end
        grid(params.ax,"on"); xlabel(params.ax,'1^{st} Objective'); ylabel(params.ax,'2^{nd} Objective');
        drawnow;
        title(params.ax,"Generation " + string(gen) + "/" + string(maxgen))
    end
    if(size(POS_fit,2)>=3) && params.plot_results
%             figure(h_fig); delete(h_par); delete(h_rep); 
        plot3(params.ax,POS_fit(:,1),POS_fit(:,2),POS_fit(:,3),'or');
        hold(params.ax,"on");
        plot3(params.ax,REP.pos_fit(:,1),REP.pos_fit(:,2),REP.pos_fit(:,3),'ok');
        hold(params.ax,"on");
%             try
%                 set(params.ax,'xtick',REP.hypercube_limits(:,1)','ytick',REP.hypercube_limits(:,2)','ztick',REP.hypercube_limits(:,3)');
%                 axis(params.ax,[min(REP.hypercube_limits(:,1)) max(REP.hypercube_limits(:,1)) ...
%                       min(REP.hypercube_limits(:,2)) max(REP.hypercube_limits(:,2)) ...
...
%             end
        if(isfield(MultiObj,'truePF'))
%                 try delete(h_pf); end
            plot3(params.ax,MultiObj.truePF(:,1),MultiObj.truePF(:,2),MultiObj.truePF(:,3),'.','color',0.8.*ones(1,3)); 
            hold(params.ax,"on");
        end
        grid(params.ax,"on"); xlabel(params.ax,'1^{st} Objective'); ylabel(params.ax,'2^{nd} Objective'); zlabel(params.ax,'3^{rd} Objective');
        drawnow;
        title(params.ax,"Generation " + string(gen) + "/" + string(maxgen))
    end
    if params.verbose
...
%                 try delete(h_pf); end
    ...
    x_pareto = REP.pos;
    fval_pareto = REP.pos_fit;

    x_all = [REP.pos; POS];
    fval_all = [REP.pos_fit; POS_fit];
